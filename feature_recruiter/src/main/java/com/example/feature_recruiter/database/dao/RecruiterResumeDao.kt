...
interface RecruiterResumeDao {
    ...
    // ✅ FIX: Better tech matching - check each tech separately
    @Query("""
        SELECT DISTINCT r.* FROM recruiter_resumes r
        WHERE r.recruiterEmail = :email 
        AND r.techStack LIKE '%' || :tech || '%'
        ORDER BY r.uploadedDate DESC
    """)
    ...
    @Query("""
        ...
        WHERE r.recruiterEmail = :email 
        AND r.techStack LIKE '%' || :tech || '%'
        AND r.experience >= :minExp 
        AND r.experience <= :maxExp
        ORDER BY r.uploadedDate DESC
    """)
