...
class ResumeRepository(private val context: Context) {
    ...
    suspend fun uploadResume(
        ...
        try {
            ...
            val userEmail = LocalResumeDatabase.getCurrentUser() ?: return@withContext Result.failure(Exception("No user logged in"))
            ...
            Log.d(TAG, "✅ Resume uploaded successfully for user: $userEmail")
            Result.success(resume)
        } catch (e: Exception) {
            Log.e(TAG, "❌ Upload failed: ${e.message}")
            Result.failure(e)
    ...
    suspend fun analyzeResume(resume: Resume): Result<ATSAnalysisResult> = withContext(Dispatchers.IO) {
        try {
            ...
            // Save analysis result to Room DB
            // ✅ Add userEmail to AnalysisResultEntity
            val analysisEntity = AnalysisResultEntity(
                resumeId = resume.id,
                userEmail = userEmail,  // ✅ ADD THIS
                overallScore = analysisResult.overallScore,
            ...
            // Save suggestions to DB with userEmail
            // ✅ Add userEmail to SuggestionEntity
            val suggestionEntities = analysisResult.suggestions.map { suggestion ->
                SuggestionEntity(
                    id = suggestion.id,
                    resumeId = resume.id,
            ...
            // ✅ Get user email for error case
            val userEmail = LocalResumeDatabase.getCurrentUser() ?: ""
